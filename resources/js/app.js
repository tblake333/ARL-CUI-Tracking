...
class TypeWriter {
  ...
  constructor(input, list, wait = 3000) {
    this.input = input;
    this.current = '';
...
class DigitWriter extends TypeWriter {
  constructor(input, wait = 3000) {
    super(input, [], wait);
    this.text = this.randDigits(Math.floor(Math.random() * 3) + 4);
  }
